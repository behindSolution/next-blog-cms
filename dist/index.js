...
function loadUserConfig() {
  ...
  for (const candidate of candidates) {
    const filePath = path2.resolve(cwd, candidate);
...
function resolveDatabasePath() {
  ...
  return path2.resolve(process.cwd(), DEFAULT_DB_FILENAME);
}
...
var languageCodeSchema = z.string().min(2).max(5).regex(/^[a-z]{2,5}$/i, "Invalid language code");
var blogStatusSchema = z.enum(["draft", "published"]);
var paginationSchema = z.object({
  page: z.coerce.number().int().min(1).optional(),
  limit: z.coerce.number().int().min(1).max(100).optional()
});
var postTranslationSchema = z.object({
...
var categoryTranslationSchema = z.object({
  language: languageCodeSchema,
  name: z.string().min(1),
...
var optionalDatetime = z.string().refine((value) => {
...
var upsertLanguageSchema = z.object({
  code: languageCodeSchema,
  name: z.string().min(1),
  enabled: z.boolean().optional(),
  isDefault: z.boolean().optional()
});
var updateLanguageSchema = z.object({
  name: z.string().min(1).optional(),
  enabled: z.boolean().optional(),
  isDefault: z.boolean().optional()
});
...
export { PostCard, PostList, blogApiHandler, defaultBlogConfig, getBlogConfig, useCategories, useLanguages, usePost, usePosts };
